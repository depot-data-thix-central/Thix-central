          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(
                children: const [
                  SizedBox(height: 16),
                  // Requested: place the search bar between the blue header band and the 4 quick buttons.
                  _SearchBar(),
                  SizedBox(height: 14),
                  _QuickActionsRow(),
                  SizedBox(height: 28),
...
class _HeaderBlock extends StatelessWidget {
  ...
  static const _screenHPadding = 20.0;
  // Header height (blue band). Search is now outside the header.
    ...
    return ClipRRect(
      ...
      child: Stack(
        children: [
          ...
          Padding(
            // Slightly taller header band (per request)
...
class _WorldMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Minimal “world map” silhouette (procedural paths) to avoid external assets.
    // Tuned for a subtle, premium backdrop once blurred by parent ImageFiltered.
    ...
    // A few soft “dots” to mimic a modern map texture.
...
class _QuickActionsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: const [
        Expanded(child: _QuickAction(icon: Icons.qr_code_scanner, label: 'Scanner QR', color: AppColors.primaryBlue)),
        SizedBox(width: 12),
        Expanded(child: _QuickAction(icon: Icons.nfc, label: 'NFC', color: AppColors.successGreen)),
        SizedBox(width: 12),
        Expanded(child: _QuickAction(icon: Icons.description_outlined, label: 'Documents', color: AppColors.accentPurple)),
        SizedBox(width: 12),
        Expanded(child: _QuickAction(icon: Icons.verified_user_outlined, label: 'Sécurité', color: AppColors.accentOrange)),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return GridView.count(
      ...
      children: const [
        ...
        _ServiceTile(icon: Icons.shopping_bag, iconColor: AppColors.accentOrange, title: 'THIX Market', subtitle: 'Achetez, vendez, profitez', route: '/market'),
        _ServiceTile(icon: Icons.school, iconColor: AppColors.accentGreen2, title: 'Formations', subtitle: 'Apprenez, progressez'),
        _ServiceTile(icon: Icons.work, iconColor: AppColors.accentPurple, title: 'Emplois', subtitle: 'Trouvez votre prochain job'),
        _ServiceTile(icon: Icons.groups, iconColor: AppColors.accentBlue2, title: 'Réseau Pro', subtitle: 'Développez votre réseau'),
        _ServiceTile(icon: Icons.event, iconColor: AppColors.dangerRed, title: 'Événements', subtitle: 'Participez, réservez'),
        ...
        _ServiceTile(icon: Icons.favorite, iconColor: AppColors.dangerRed, title: 'THIX Santé', subtitle: 'Prenez soin de vous'),
        _ServiceTile(icon: Icons.feed, iconColor: AppColors.accentOrange, title: 'THIX INFO', subtitle: 'Actualités & informations'),
        _ServiceTile(icon: Icons.account_balance, iconColor: AppColors.accentBlue3, title: 'Services Gov', subtitle: 'Services publics en ligne'),
        _ServiceTile(icon: Icons.confirmation_num, iconColor: AppColors.accentPurple, title: 'Réservation', subtitle: 'Billets, hôtels, voyages'),
      ],
    );
  }
}
...
class _ServiceTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final t = Theme.of(context).textTheme.labelSmall?.copyWith(color: AppColors.darkNavy, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: -0.2, height: 1.05);
...
class _PromoCarouselState extends State<_PromoCarousel> {
  ...
  static const _autoScrollEvery = Duration(seconds: 1);
...
class _PromoBannerCard extends StatelessWidget {
  ...
  String? _routeForIndex(int index) {
    // Today we only wire THIX Market (first banner). Others can be connected later.
    if (index % 4 == 0) return '/market';
    return null;
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 4 “vraies” bannières pub (contenus différents). Sans backend, ce sont des placeholders UI.
    final items = const [
...
// _PhoneMockups removed (replaced by _PromoArt)
