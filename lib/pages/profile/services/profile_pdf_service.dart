...
class ProfilePdfService {
  ...
  Future<void> printReceipt({required String displayName, required String thixId, required ProfileTransactionModel txn}) async {
    final doc = pw.Document();
          ...
          return pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
              pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                pw.Text('Reçu', style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 4),
                pw.Text('THIX ID: $thixId'),
                pw.Text('Client: $displayName'),
              ]),
              pw.Container(
                padding: const pw.EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                decoration: pw.BoxDecoration(color: PdfColors.green50, borderRadius: pw.BorderRadius.circular(18)),
                child: pw.Text(txn.status.toUpperCase(), style: pw.TextStyle(color: PdfColors.green800, fontWeight: pw.FontWeight.bold)),
              ),
            ]),
            ...
            pw.Container(
              ...
              child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
                pw.Text('Transaction', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 10),
                pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('Type'), pw.Text(txn.type)]),
                pw.SizedBox(height: 6),
                pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('Montant'), pw.Text('\$${txn.amountUsd.toStringAsFixed(2)}')]),
                pw.SizedBox(height: 6),
                pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('Méthode'), pw.Text(txn.method)]),
                pw.SizedBox(height: 6),
                pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('Date'), pw.Text(df.format(txn.createdAt))]),
                pw.SizedBox(height: 6),
                pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [pw.Text('Référence'), pw.Text(txn.id)]),
              ]),
            ),
            pw.Spacer(),
            pw.Text('Document généré par THIX (paiement simulé).', style: pw.TextStyle(color: PdfColors.grey600, fontSize: 10)),
          ]);
        },
      ),
    );
